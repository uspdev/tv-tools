...
[ ! -e /etc/lightdm.conf ] && cp ./lightdm.conf /etc/lightdm.conf;
[ ! -e /etc/lightdm.conf.d ] && mkdir /etc/lightdm.conf.d && cp ./50-myconfig.conf /etc/lightdm.conf.d;
read -p "Informe o nome de usuário, por favor (enter para usuário padrão)" username


if([ -z "$username" ])
then
	sed -i "s/tvpc/$username/g" /etc/lightdm.conf.d/lightdm.conf.d/50-myconfig.conf;
	sed -i "s/tvpc/$username/g" /etc/lightdm.conf
fi
